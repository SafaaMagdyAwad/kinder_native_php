  ...
  if(
    ...
    &&isset($_FILES['image']) && !empty($_FILES['image'])
     ...
     if(isset($_POST['published']) && !empty($_POST['published'])) {
      ...
      try{
        $sql = "INSERT INTO `testimonials`( `fullName`, `jopTitle`, `comment`, `published`, `image`) VALUES (?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$fullName,$jopTitle,$comment,$published,$image_name]);
      ...
      $error= "Full name, jop Title, comment,image are required";
    }
